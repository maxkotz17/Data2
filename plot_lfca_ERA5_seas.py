...
LFC=[]
for i in range(4):
	LFP.append(np.load(folder + seas + '_' + num + '_var_T_cont_LFP_' + str(i) + '.npy').transpose())
	LFC.append(np.load(folder + seas + '_' + num + '_var_T_cont_LFC_' + str(i) + '.npy'))
...
plt.tight_layout()
plt.savefig('LFCA_plots/ERA5/' + seas + '_' + num + 'ERA5_panel.png',bbox_inches='tight')
plt.close()
...
LFP_decades=np.zeros((LFP[1].shape[0],LFP[1].shape[1],4))

for i in range(len(decades)):        
	LFP_decades[:,:,i]=LFP[1][:,:]*np.mean(np.real(LFC[1])[(decades[i]-1979):(decades[i]-1969)])
	print(np.mean(np.real(LFC[1])[(decades[i]-1979):(decades[i]-1969)]))
...
plt.savefig('LFCA_plots/ERA5/' + seas + '_' + num + 'ERA5_lfca_decades.png',bbox_inches='tight') 
plt.close()
...
if seas=='DJF':
	selector=[True,True,False,False,False,False,False,False,False,False,False,True]
if seas=='JJA':
	selector=[False,False,False,False,False,True,True,True,False,False,False,False]
...
plt.savefig('LFCA_plots/ERA5/' + seas + ' ERA5_lfca_decades_perc_diff.png',bbox_inches='tight')
plt.close()


#ERA5 process

decade_diff=LFP[0]*(np.mean(np.real(LFC[0][-10:]))-np.mean(np.real(LFC[0][:10])))
...
plt.savefig('LFCA_plots/ERA5/' + seas + '_' + num + 'ERA5_process.png',bbox_layout='tight')
plt.close()
...
axf.set_ylim([-60,80])
axf.set_title('2009-2019 from 1979-1989, percentage change') 
...
plt.savefig('LFCA_plots/ERA5/ERA5_process2.png',bbox_layout='tight')
plt.close()
