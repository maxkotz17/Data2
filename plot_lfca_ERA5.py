...
LFC=[]
for i in range(4):
	LFP.append(np.load(folder + 'var_T_cont_LFP_' + str(i) + '.npy').transpose())
	LFC.append(np.load(folder + 'var_T_cont_LFC_' + str(i) + '.npy'))
...
for i in range(2):
	for j in range(2):
		...
		smoothed=tukey(LFC[i*2+j],0,12*7)
...
plt.tight_layout()
plt.savefig('LFCA_plots/ERA5/ERA5_panel.png',bbox_inches='tight')
plt.close()
...
plt.savefig('LFCA_plots/ERA5/ERA5_lfca_decades.png',bbox_inches='tight') 
plt.close()
