...
#load data
folder='LFCA/GSWP3-W5E5/'

LFP=[]
LFC=[]
for i in range(4):
	LFP.append(np.load(folder + 'gswp3-w5e5_var_T_cont_LFP_' + str(i) + '.npy').transpose())
	LFC.append(np.load(folder + 'gswp3-w5e5_var_T_cont_LFC_' + str(i) + '.npy'))
...
time=np.linspace(1901,2016,len(LFC[0]))		
...
plt.tight_layout()
plt.savefig('LFCA_plots/GSWP3-W5E5/GSWP3-W5E5_panel.png',bbox_inches='tight')
plt.close()
...
time=np.linspace(1900,2016,len(LFC[0]))
...
axd.set_title('2006-2016 from 1900-1910, estimated from LFC-0')
axd.set_ylim([-60,80])
...
plt.savefig('LFCA_plots/GSWP3-W5E5/GSWP3-E5W5_process.png',bbox_layout='tight')
plt.close()
