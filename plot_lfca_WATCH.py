...
LFC=[]
for i in range(4):
	LFP.append(np.load(folder + 'watch_var_T_cont_LFP_' + str(i) + '.npy').transpose())
	LFC.append(np.load(folder + 'watch_var_T_cont_LFC_' + str(i) + '.npy'))
...
plt.tight_layout()
plt.savefig('LFCA_plots/WATCH/WATCH_panel.png',bbox_inches='tight')
plt.close()
