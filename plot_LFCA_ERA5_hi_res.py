...
LFC=[]
for i in range(4):
	LFP.append(np.load(folder + 'high_res_var_T_cont_LFP_' + str(i) + '.npy').transpose())
	LFC.append(np.load(folder + 'high_res_var_T_cont_LFC_' + str(i) + '.npy'))
...
time=np.linspace(1978,2017+11/12,len(LFC[0]))		

fig=plt.figure(constrained_layout=False,figsize=(22,12))
widths=[10,10]
heights=[4,2,4,2]
spec=fig.add_gridspec(ncols=2,nrows=4,width_ratios=widths,height_ratios=heights,hspace=0,wspace=0.2)
for i in range(2):
	for j in range(2):
		...
		ax.set_ylim([-60,80])
		ax.set_title('Low Frequency Pattern ' + str(i*2+j),fontsize=17)

		smoothed=tukey(LFC[i*2+j],0,120)
...
#		ax2.set_xlabel('Time',fontsize=17)
		ax2.set_ylabel('Standard Deviations',fontsize=14)
		ax2.set_title('Low Frequency Component ' + str(i*2+j),fontsize=17)
		ax2.set_xlim([1978,2018])
		ax2.set_aspect(2)
		ax2.set_xticks([1980,1990,2000,2010])
		ax2.tick_params(labelsize=14)
		ax2.hlines(y=0,xmin=1978,xmax=2018,color='r',alpha=0.5,ls='dashed')
...
#plt.tight_layout()
plt.savefig('LFCA_plots/ERA5/ERA5_hi_res_panel.png')#,bbox_inches='tight')
plt.close()
...
for i in range(len(decades)):        
	LFP_decades[:,:,i]=LFP[0][:,:]*np.mean(np.real(LFC[0])[(decades[i]-1979)*12:(decades[i]-1969)*12])
	print(np.mean(np.real(LFC[0])[(decades[i]-1979)*12:(decades[i]-1969)*12]))
...
plt.savefig('LFCA_plots/ERA5/ERA5_hi_res_lfca_decades.png',bbox_inches='tight') 
plt.close()
...
[T_var_decs,lon,lat]=sub_sample(T_var_decades,np.linspace(1,180,180),np.linspace(1,360,360),1)
...
plt.savefig('LFCA_plots/ERA5/ERA5_hi_res_process.png',bbox_layout='tight')
plt.close()
...
widths=[10,10]
heights=[4,4]
spec=fig.add_gridspec(ncols=2,nrows=2,width_ratios=widths,height_ratios=heights,hspace=0.1,wspace=0.05)
vmin=-1
vmax=0.5
divnorm = mcolors.DivergingNorm(vmin=vmin, vcenter=0, vmax=vmax)
...
map0.drawcoastlines(ax=ax)
ax.set_ylim([-60,80])
ax.set_title('Lowest Frequency Pattern 0 (LFP-0)',fontsize=15)
...
cbar.set_ticklabels(labels)
cbar.ax.tick_params(labelsize=17)
time=np.linspace(1979,2018+11/12,len(LFC[0]))
smoothed=tukey(LFC[0],0,120)
axt=fig.add_subplot(spec[0,0])
axt.plot(time,LFC[0],alpha=0.4,c='gray')
axt.plot(time,smoothed,alpha=1,c='k')
axt.set_ylabel('Standard Deviation',fontsize=14)
axt.set_ylim([-2,2])
axt.set_xlim([time[0],2020])
axt.set_title('Lowest Frequency Component LFC-0',fontsize=15)
axt.set_aspect("auto")
axt.set_yticks([-2,-1,0,1,2])
axt.set_xticks([1980,1990,2000,2010,2020])
...
map0.pcolormesh(xx,yy,decade_diff,cmap=my_cmap,norm=divnorm,ax=axd)
map0.drawcoastlines(ax=axd)
#axd.set_title('2009-2019 from 1979-1989, estimated from LFC-0',fontsize=15)
axd.set_ylim([-60,80])
...
axf=fig.add_subplot(spec[1,1])
map0.pcolormesh(xx,yy,100*np.divide(decade_diff,T_var_decs[:,:,0]),cmap=my_cmap,norm=divnorm,ax=axf)
map0.drawcoastlines(ax=axf)
axf.set_ylim([-60,80])
#axf.set_title('2009-2019 from 1979-1989, percentage change',fontsize=15)
axins=inset_axes(axf,width='66%',height='5%',loc='lower center',bbox_to_anchor=(0,-0.1,1,1),bbox_transform=axf.transAxes,borderpad=0,)
sm = plt.cm.ScalarMappable(cmap=my_cmap, norm=divnorm)
cbar=fig.colorbar(sm,cax=axins,orientation='horizontal')
cbar.set_ticks([vmin,0,vmax])
...
cbar.set_ticklabels(labels)
cbar.ax.tick_params(labelsize=15)
plt.savefig('LFCA_plots/ERA5/ERA5_hi_res_process2.png')
plt.close()
